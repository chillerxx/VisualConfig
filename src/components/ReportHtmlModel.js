...
const All = ({ reportsData, reportConfigedElems, renderCustomComponent, afterReportRender, beforeReportRender, onTable }) => {
  ...
  const parseNormalTableData = (elems, reportData) => {
    ...
    elems.forEach((elem, i) => {
      ...
      // let isCustom = elem.component === 'customComponent'
      // const ySpan = (isCustom ? Math.ceil(height / gridH) : Math.round(height / gridH)) || 1
      // const xSpan = (isCustom ? Math.ceil(width / gridW) : Math.round(width / gridW)) || 1
